    ...
    // Enemy-enemy bullet collision (for friendly fire if applicable)
      ...
      spawnEnemy() {
          ...
          if (!this.enemyTypes || this.enemyTypes.length === 0) {
            console.log("No enemy types defined!");
            return; // Exit the function if no enemy types defined
          }
        
          const totalWeight = this.enemyTypes.reduce((acc, enemyType) => acc + enemyType.weight || 1, 0); // Calculate total weight (handling missing weight)
          ...
          for (let i = 0; i < this.enemyTypes.length; i++) {
            accumulatedWeight += this.enemyTypes[i].weight || 1; // Handle missing weight with default of 1
          ...
          const chosenEnemyType = this.enemyTypes[chosenTypeIndex];
          const randomSpawnPoint = Phaser.Math.RND.pick(this.spawnPoints);
        
          const enemy = this.physics.add.sprite(randomSpawnPoint.x, randomSpawnPoint.y, `${chosenEnemyType.animationPrefix}1`);
